...
import { Search, Users, UserRound, Building2, ChevronRight } from "lucide-react";
  ...
  return (
    <Card className="space-y-2.5">
      <div className="flex items-start justify-between gap-2">
        <Link href={`/admin/u/${u.id}`} className="min-w-0 flex-1">
...
function ClientRow({ c }: { c: AdminClient }) {
  return (
    <Link href={`/admin/u/${c.id}`} className="block">
      <Card className="flex items-center justify-between gap-2 transition active:scale-[0.99]">
