    ...
    <div>
      <div className="mb-2 flex items-center justify-between">
        <h2 className="text-sm font-bold text-foreground">
          {tr("Beneficiarios")} ({associated.length})
        </h2>
        <button
          type="button"
        ...
        >
          <Plus className="h-3.5 w-3.5" /> {tr("Añadir")}
        </button>
      </div>

      {associated.length === 0 ? (
        <Card className="text-center text-sm text-muted-foreground">
          {tr("Sin beneficiarios asociados todavía.")}
        </Card>
      ) : (
      ...
      >
        <div className="mb-3 flex items-center gap-2 rounded-xl border border-input bg-background px-3 py-2">
          ...
          <input
            ...
            placeholder={tr("Buscar por nombre o provincia")}
        ...
        {filtered.length === 0 ? (
          <p className="py-6 text-center text-sm text-muted-foreground">
            {available.length === 0
              ? tr("No hay más beneficiarios en tu agenda.")
              : tr("Sin resultados.")}
          </p>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-muted-foreground">
                      {[b.province, otherClient ? `${tr("de")} ${otherClient}` : null]
                        .filter(Boolean)
                        .join(" · ") || tr("Sin provincia")}
                    </p>
                  </div>
