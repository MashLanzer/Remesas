...
export function EnRouteToggle({
  ...
  const digits = beneficiaryPhone?.replace(/\D/g, "") || "";
  const waHref = digits
    ? `https://wa.me/${digits}?text=${encodeURIComponent(
        `Hola${
          beneficiaryName ? ` ${beneficiaryName}` : ""
        }, voy en camino con tu envío. Llego en un rato. 🚗`
      )}`
    : null;
      ...
      <div className="mb-3 space-y-2 rounded-2xl border border-info/25 bg-info/5 p-3.5">
        ...
        {waHref && (
          <a
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-income/10 py-2.5 text-sm font-semibold text-income transition active:scale-[0.98]"
          >
            <MessageCircle className="h-4 w-4" /> Avisar “voy en camino”
          </a>
        )}
      </div>
    );
